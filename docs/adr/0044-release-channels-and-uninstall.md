...
1. **通道集定案（BLK-02=① 落档）**：M5 生效通道=**npm（`@standardcode/cli`，全平台首选）+ 三平台安装/手动清理脚本 + checksum**。二进制（Bun compile/Node SEA）=骨架与文档就位（构建脚本位登记，不实发；Q-5 倾向"npm → 双通道演进"序进读法=双通道为 M5 后演进项）；winget/直接下载/Homebrew/deb|rpm=仅本 ADR 登记占位，零实现（B-03：进不了本 M 的不做）。
