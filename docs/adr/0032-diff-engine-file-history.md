...
4. **/rewind N 语义**：撤销 seq>N 的全部快照（后进先出）：恢复最近一次 ≤N 的存档内容，或（`existed:false`）删除新建文件。索引只追加不改写（历史保留）。**会话消息历史不随 rewind 截断**（M2 无 per-snapshot 消息检查点）——transcript 只读不破坏，会话级回退登记为后续里程碑缺口。
