...
**统一原则**：
1. **降级不降安全**【自定，综合判断】——安全相关通道（快照、护栏、权限）缺依赖即拒绝执行（fail-closed），数据通道（transcript）可降级继续（fail-open 限单文件，ADR-0030 决策 4 同构）。
2. **一切降级可观测**——degraded 标志/告警/遥测事件（ENG-090），禁止静默吞错。
3. **用户可恢复**——每面给出建议动作（错误三件套：发生了什么/为什么/建议）。

## §12.5 差异登记

三面处理均为 M1 实证回指（providers retry/session-store 降级/permission-broker），非新引用外部实现——无新增差异条目；"统一原则①"为跨面综合 [自定]（见上）。
