...
2. **Q-6「Bun 作运行时的取舍」→ 结案：维持"仅作打包器"定位；M7 期**预授权**=Bun compile（条件性，**未最终定案**），定案落点=BLK-09 的 WP-09 开工 mini-ADR。**
   - 事实锚：M5 复审（ADR-0044 决策 2）= **M5 期不引入 bun**（打包器=esbuild 单文件 ESM bundle，零运行时依赖，bun 未安装）；M7 BLK-09（M7-1 板阻塞卡）= 独立二进制选型二选一（Bun compile〔P 倾向〕vs Node SEA），**用户 2026-09-19 附条件预授权原话=「暂时没想好；未完工继续推进；需定案时选 Bun」**（引号内逐字；预授权指向的选型=Bun compile；用户可改选）。
