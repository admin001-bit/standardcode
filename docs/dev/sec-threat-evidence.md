...
| S-5 | Skill/Plugin 投毒 | 安装确认+清单展示+allowed-tools 白名单 | 技能信任门前置（SEC-070）：`packages/capabilities/src/skills/discovery.ts`（trusted 参数）；allowed-tools 白名单收窄（toolFace，Skill 恒保留）：`apps/cli/src/session.ts:748-756`；plugin 安装确认门先于任何写盘：`packages/platform/src/plugin/installer.ts:186`〔勘误 2026-09-17：原引 189 系 M4 核验时点行号，落表时实为 186，V 裁书写级漂移〕；清单展示=`apps/cli/src/repl.ts`（/skills /plugin list 面） | `packages/capabilities/test/skills.test.ts`+`apps/cli/test/wp05-skills-session.test.ts`+`packages/platform/test/wp09-plugin.test.ts`〔勘误 2026-09-17：原引目录前缀笔误，V 亲验文件实在 platform/test〕 |
