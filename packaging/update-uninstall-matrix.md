...
| **`reg` 注册表 PATH 还原**（`defaultPathRestorer` win 分支）〔**2026-09-26 复验（M8-WP-09）：可得（免提权）→ 已写-还原实跑，暴露 D-1**〕 | **已实跑（部分：移除通道 ✓／保真 ✗）** | — | — | 事实订正：HKCU\Environment 用户级**免提权**（原记"待管理员机"过严）。实跑：探针项写入→真调还原器→`removed:[探针]` ✓；**缺陷 D-1（低-中危，新）**＝`reg query` 输出按 `utf8` 解（实为控制台代码页/GBK）⇒ **含非 ASCII 段的 PATH 回写有损**（本机实测 `D:\OCR???` 形态；见根 `.work/M8-X-WP09-report.md` §0）。修法建议＝读侧按代码页解码或跳过非 ASCII 段回写（fail-closed）＋保真用例；**该面保真判据在修复前不成立**（写路径暂停） |
