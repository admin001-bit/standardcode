...
| **`--purge` fail-closed**（非 TTY 无确认通道=拒绝） | 已验证 | 已验证 | 已验证 | `wp07-release-cli.test.ts`（非 TTY 拒绝 exit 1 + 目录保留；用户拒答 aborted）——平台无关。**2026-09-26（M8-WP-08/V）新增 D-V1**：真 bin（`bin/standardcode.js:19`／`.binary.mjs:25`）均以**单参**调 `runUninstall` ⇒ `io.confirm` 从未装配 ⇒ 真 bin 下 `--purge` **恒走本条拒绝路**（含 TTY＝交互路径不可用；④ 段引导文案指向不可达路径）。fail-closed 安全面成立（无数据删除）；功能面缺口＝交互 purge 通道未装配。**〔2026-09-26 晚（M8-WP-11）D-V1 已修〕**：新增 `runUninstallCli` 入口（TTY 判定→readline y/N 装配默认确认；非 TTY 不装配＝本条拒绝语义零改），bin 两入口改调；判据 `apps/cli/test/wp11-uninstall-purge-confirm.test.ts`（TTY 确认→purge 全链／非 TTY 仍拒绝／拒答中止）＋**真进程** `node apps/cli/bin/standardcode.js uninstall --purge`（非 TTY）→ 本条拒绝 rc=1（沙箱前缀实跑）。**〔V 核验新增 D-V1′（低危，2026-09-26；main 亲复现）〕**：TTY 态 stdin **纯 EOF**（无输入即关闭）→ `rl.question` 不 settle → bin 顶层 await 未决 **rc=13**（`printf '' | node …` 直证）；**无删除发生**（fail-closed 安全面成立）但无 `purge aborted` 文案；修法建议＝readline `close` 竞速 `resolve(false)` 走 aborted＋rc=1。**〔2026-09-26 追加修复（M8-WP-11，用户裁决①）〕D-V1′ 已修**：默认确认抽为可注入工厂 `createTtyConfirm(input, output)`（`close` 竞速 `resolve("")`＝否，不悬挂），`runUninstallCli` 用之；判据＝`wp11-uninstall-purge-confirm.test.ts` 新增工厂直测两例（y/YES→true、n/空行→false；**纯 EOF→false 不悬挂**）＋真进程复跑（`printf '' | node …` → `purge aborted by user`＋rc=1，原 rc=13 消失）；MUT 针（去 close 竞速）→ EOF 例恰 1 红（超时）、复原 sha256 字节级一致 |
| **手动清理脚本**（`scripts/cleanup.ps1 -PurgeHome` / `scripts/cleanup.sh --purge-home`） | 静态已验证 | 静态已验证 | 静态已验证 | 三平台用例内静态断言：脚本含对应旗标、包名与 `NPM_PACKAGE_NAME` 同字（`@standardcode-oss/cli`）、且不含改判前旧名；**实跑未验证**（原记 spawnSync EBUSY／wsl.exe 拦截——两约束已随 §0 失效；实跑本身仍未做〔**2026-09-26（M8-WP-10）同步，承 V 的 D-V4**〕） |
| **`reg` 注册表 PATH 还原**（`defaultPathRestorer` win 分支）〔**2026-09-26 复验（M8-WP-09）：可得（免提权）→ 已写-还原实跑，暴露 D-1**；**同日晚（M8-WP-12，用户裁决①）D-1 已修**〕 | **已实跑（移除通道 ✓；保真 ✗ 已于 M8-WP-12 修复）** | — | — | 事实订正：HKCU\Environment 用户级**免提权**（原记"待管理员机"过严）。实跑：探针项写入→真调还原器→`removed:[探针]` ✓；**缺陷 D-1（低-中危，新）**＝`reg query` 输出按 `utf8` 解（实为控制台代码页/GBK）⇒ **含非 ASCII 段的 PATH 回写有损**（本机实测 `D:\OCR???` 形态；见根 `.work/M8-X-WP09-report.md` §0）。修法建议＝读侧按代码页解码或跳过非 ASCII 段回写（fail-closed）＋保真用例；**该面保真判据在修复前不成立**（写路径暂停）。**〔2026-09-26（M8-WP-12）D-1 已修〕**：读侧改 `cmd /c chcp 65001>nul & reg query …` 包装（子进程控制台代码页钉为 UTF-8；只读对照 C0 有损 `???` vs E1 保真 `ʶ`＋U+FFFD×2 逐字符、len=1036；`.work/M8-X-WP12-report.md`）；判据 `apps/cli/test/wp12-path-restore-fidelity.test.ts`（读命令形／解析逐字符／fail-closed 零写／写回载荷 `/d` 逐字符／win32 只读实证 ≡ `reg export` hex(2) 独立通道）；MUT 针（去 `chcp`）恰 2 红、复原 sha 字节级一致；写路径宽字符保真旁证＝事故实证中 `reg add` 已原样存 U+02B6+FFFD×2（"读×写"保真链）。**不采用**：PowerShell `Get-ItemProperty`（缺省展开 `%USERPROFILE%`＝语义改变，E3 实测 len 1037）；`reg export` UTF-16LE 通道（可用但需多行 hex(2) 解析，留作判据 oracle） |
...
| **winget / Homebrew 卸载**〔**2026-09-26 复验（M8-WP-09）**：winget 在位 v1.29.380（Git Bash PATH 不含，需全路径）；`winget validate packaging/winget/manifests` **rc=0「清单验证成功」**（免提权可得面实跑 ✓）；真机安装链仍不可得（`LocalState\Settings\settings.json` 不存在＝`localManifestFiles` 缺省禁用）——用户侧动作＝管理员 `winget settings --enable LocalManifestFiles`；brew 无 macOS ⇒ 不可得〕 | 本机不可验证（需管理员 LocalManifestFiles / 无 brew） | — | 待 CI | 承 WP-10 BLK-12 未验证面登记 |
