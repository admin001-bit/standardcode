...
describe("completeInput (UI-001)", () => {
  ...
  it("多义前缀 → 候选列表；单命令前缀 → 唯一补全", () => {
    ...
    expect(c.candidates).toEqual(["/help", "/clear", "/exit", "/model", "/permission", "/rewind", "/context", "/diff", "/new", "/resume", "/rename", "/compact", "/config", "/provider", "/doctor", "/cd", "/add-dir", "/reload", "/tasks", "/background", "/subtask", "/effort", "/init", "/status", "/usage", "/mcp", "/skills", "/memory", "/plugin", "/update", "/goal", "/theme"]); // 30（2026-09-15）+M7-WP-01 /goal=31（2026-09-19）+M7-WP-03 /theme=32（2026-09-23）
