...
export function buildMemoryDisciplinePrompt(opts?: { autoTrackEnabled?: boolean }): string {
  ...
  return [
    ...
    "",
    "1. Write each memory as its own file (`<slug>.md`, e.g. `feedback_testing.md`) using this frontmatter format:",
    "",
    "```markdown",
    "---",
    ...
    "### Memory types",
    "",
    "- **user** - the person you work with: their role, preferences, and working style",
    "- **feedback** - corrections about how you work; capture the correction and where to apply it next time",
    "- **project** - durable facts about this codebase: architecture decisions, build and test commands, conventions",
    "- **reference** - pointers to external material worth revisiting",
    "",
    "### Linking and staleness",
    "",
    "In memory bodies, link related memories with `[[name]]` (the other memory's slug). A `[[name]]` that points at a memory which does not exist yet is acceptable - treat it as a placeholder for something you plan to write later, not as a failure.",
    "",
    "Memory entries go out of date. Treat each entry as a snapshot of an earlier moment: before relying on one, re-check it against the code or data as they are now; when a memory disagrees with what you currently observe, prefer what you observe and correct or remove the outdated record.",
  ].join("\n");
}
