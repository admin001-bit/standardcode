  ...
  while (true) {
    ...
    if (signal?.aborted) {
      ...
      if (pending.length > 0) {
        ...
        const outcomes = await runTools(pending, { registry, permission: opts.permission, guard: opts.guard, fileHistory: opts.fileHistory, hooks: opts.hooks, signal, onProcess: opts.onProcess, agentKind: opts.agentKind });
